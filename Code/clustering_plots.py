...
def plotting(dimensions, model_name, s_path):
    ...
    if '2d' in dimensions:  # Check if 2D or 3D 
        ...
        for i in range(0, len(plots)):  # Loop over all combos
            ...
            colour1_2d, colour2_2d, wave = load_2d_cluster_data(cluster_data_2d,
                                                          waves)
...
def make_2d_plots(c1, c2, bands, n_clust, alg, c_data, centers, path,
    ...
    ax.set_ylabel(bands[2]+' - '+bands[3])
    ax.set_title(alg + ' ' + str(n_clust) + ' : '+bands[0]+'-'+bands[1]+' vs. '+bands[2]+'-'+bands[3],
                 fontsize=14)
...
def make_3d_plots(c1, c2, c3, bands, n_clust, alg, c_data, centers,
    ...
    for i in range(1, 3):
        ...
        ax.set_title(alg + ' ' + str(n_clust) + ': '+bands[0]+'-'+bands[1]+' vs. '+bands[i*2]+'-'+bands[i*2+1],
                     fontsize=14)
    ...
    for b in range(0, n_clust):
        ...
        ax2.scatter(c1[objects], base_colour[objects], marker='o',
    ...
    ax2.set_title(alg + ' ' + str(n_clust) + ' : '+bands[0]+'-'+bands[1]+' vs. '+bands[base1]+'-'+bands[base2],
                  fontsize=14)
    ax2.legend(loc='lower right', fontsize=8)
